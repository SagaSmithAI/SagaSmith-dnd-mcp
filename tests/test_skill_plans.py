...
def test_skill_plan_manifest_covers_every_tool_group_and_tracks_reads(
    ...
    assert plans.summary()["tool_group_count"] == len(GROUP_BY_ID) == 24
    plan = plans.plan(
        phase="lobby",
        role="public",
...
def test_real_skill_plan_manifest_is_valid_and_within_budgets() -> None:
    ...
    assert plans.summary()["group_count"] == 39
    assert plans.summary()["tool_group_count"] == 24
    ...
    async def exercise() -> None:
        ...
        async with stdio_client(params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                assert summary["tool_group_count"] == 24
